...
class _RoutingBody extends StatelessWidget {
  ...
  String get _modeHint => switch (routing.direction) {
    ...
    RoutingDirection.bypass =>
      '$_ruleCount правил · совпавшие напрямую, остальное через VPN',
    RoutingDirection.onlySelected =>
      '$_ruleCount правил · только совпавшие через VPN',
  };
  ...
  Widget build(BuildContext context) {
    ...
    final geoHint = geoMissing
        ? 'Загрузите geo-базы (Настройки → geo.data)'
        : null;
              ...
              children: [
                ...
                if (routing.isActive) ...[
                  ...
                  _RowToggle(
                    ...
                    title: 'Снифинг',
                    hint:
                        'Определять домен из TLS SNI · нужен для domain.suffix и geosite',
                  ...
                  _ToggleWithChips(
                    ...
                    last: false,
                    onToggle: (v) =>
                        onUpdate(routing.copyWith(geositeEnabled: v)),
                    chips: routing.geositeCodes,
                    onRemove: geositeLocked
                        ? null
                        : (code) => onUpdate(
                            routing.copyWith(
                  ...
                  _ToggleWithChips(
                    ...
                    last: true,
                    onToggle: (v) =>
                        onUpdate(routing.copyWith(sitesEnabled: v)),
                    chips: routing.sites,
                    onRemove: domainLocked
                        ? null
                        : (site) => onUpdate(
                ...
                _RowToggle(
                  ...
                  hint:
                      geoHint ??
                      'geosite:category-ads-all + geosite:win-spy → block',
                  value: routing.adBlockEnabled,
                  locked: locked || geoMissing,
                  onChange: (v) =>
                      onUpdate(routing.copyWith(adBlockEnabled: v)),
                ),
                _RowToggle(
                  t: t,
                  title: 'Российские сервисы',
                  hint:
                      'Яндекс, VK, Сбер, Ozon, Авито и др. → выбранный outbound',
                  value: routing.ruServicesEnabled,
                  locked: locked,
                  last: true,
                  onChange: (v) =>
                      onUpdate(routing.copyWith(ruServicesEnabled: v)),
                ),
                  ...
                  child: Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Text(
                          GeoPresets.nameOf(geoipUrl, geositeUrl),
                ...
                children: [
                  ...
                  _CustomInputRow(
                    ...
                    onAdd: () {
                      ...
                      if (code.isNotEmpty) {
                        ss(() => selected.add(code.toLowerCase()));
                        customCtrl.clear();
                      }
                    },
                  ),
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, ss) {
          final t = Theme.of(ctx).extension<TeapodTokens>()!;
          void add() {
            final zone = customCtrl.text.toLowerCase().trim().replaceAll(
              RegExp(r'^\.+'),
              '',
            );
                ...
                children: [
                  for (final (zone, label) in _popularDomains)
                    _CheckRow(
                      ...
                      onChanged: (v) => ss(() {
                        v ? selected.add(zone) : selected.remove(zone);
                      }),
                    ),
                  for (final zone in customDomains)
                    _CheckRow(
                      t: t,
                      title: _formatDomainLabel(zone),
                      subtitle: zone,
                      value: true,
                      onChanged: (v) {
                        if (v == false)
                ...
                children: [
                  for (final site in _popularSites)
                    _CheckRow(
                      t: t,
                      title: site,
                      subtitle: 'domain:$site',
                      value: selected.contains(site),
                      onChanged: (v) => ss(() {
                        v ? selected.add(site) : selected.remove(site);
                      }),
                    ),
          ...
          children: opts.asMap().entries.map((e) {
            ...
            return Expanded(
              child: GestureDetector(
                onTap: locked
                    ? null
                    : () => onUpdate(routing.copyWith(direction: dir)),
                child: Container(
                ...
                children: [
                  for (var i = 0; i < chips.length; i++)
                    _SquareChip(
                      ...
                      onTap: onRemove != null
                          ? () => onRemove!(
                              chipKeys != null ? chipKeys![i] : chips[i],
                            )
                          : null,
                      showDelete: true,
                    ),
...
class _NotchTick extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomPaint(
    size: const Size(5, 5),
    painter: _NotchPainter(color, topLeft),
  );
}
